...
# Put in placeholder matrix until we have one produced
exNWave, exNtime = 10, 3000
fluxTimeWavelengthExample = np.ones([Ncolumns, exNtime, exNWave])

for waveInd in np.arange(exNWave):
    fluxTimeWavelengthExample[timeColumn, :, waveInd] = np.linspace(-2000,
                                                                    2000,
                                                                    exNtime)
for tInd in np.arange(exNtime):
    fluxTimeWavelengthExample[waveColumn, tInd, :] = np.linspace(2.4,
                                                                 4,
                                                                 exNWave)
...
def add_noise(fluxTWave, preserveInput=True, nEclipses=5,
    ...
    preserveInput: bool
        ...
        The rationale is that this way your posteriors should have a median
        close to the true Input. Otherwise, you'd have to run the fits multiple
        times to test if the median posterior was centered on your true input
        solution
    ...
    dat = gn.make_snr_spectrum(planet)
    texp = dat['texp']  # seconds

    nColumns, nTime, nWave = fluxTWave.shape

    minTime, maxTime = [np.min(fluxTWave[timeColumn, :, :]),
                        np.max(fluxTWave[timeColumn, :, :])]

    timeStarts = np.arange(minTime, maxTime-texp, texp)
    timeEnds = timeStarts + texp
    timeMid = (timeStarts + timeEnds)/2.

    outDict = {"time (days)": timeMid / (3600.*24.),
               "wavelength (um)": fluxTWave[waveColumn, 0, :],
    ...
    for waveInd in np.arange(nWave):
        ...
        wavePts = (dat['wave'] > waveMid - dWave) & \
                  (dat['wave'] <= waveMid + dWave)

        if np.sum(wavePts) >= 1:
            combinedErr = np.sqrt(np.sum(dat['sigma_ppm']**2)) / \
                          np.sum(wavePts)

            for tInd, oneTime in enumerate(timeMid):
                timeArray = fluxTWave[timeColumn, :, waveInd]
                inPts = (timeArray > timeStarts[tInd]) & \
                        (timeArray <= timeEnds[tInd])
            ...
            if renormalize is True:
                t['flux (ppm)'] = 1e6 * t['flux (ppm)'] / \
                                  np.min(t['flux (ppm)'])

            if writeCSVs:
                t.write('data/output_lightcurves/{}.csv'.format(baseName),
                        overwrite=True)
            ...
            if doPlots is True:
                ...
                fig.savefig('data/output_lightcurves/plots/{}.pdf'.format(
                            baseName))
